...
- **原因**：2026-09-22 复查：窗口实际已创建（CGWindowList 可见 900×500 主窗口），但包括菜单栏在内全部 `onscreen=false`，直接运行二进制也一样 → 大概率是当时屏幕锁定 / 不在当前桌面空间，**不是 App 的问题**。维护者手动打开能看到窗口即可关闭本条。
...
- **解法**（✅ 2026-09-22 已在 App 内完成，前端未改、只需 Key）：地域可选（北京 `dashscope.aliyuncs.com` / 新加坡 `dashscope-intl.aliyuncs.com`），协议按 `scripts/phase0/probe.py` 已验证的版本。参考实现：`huankechong/QwenLiveTranslate`（MIT）的 `realtime_client.py`。
